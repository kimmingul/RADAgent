...
RAD Agent는 omp를 띄울 때 활성 Delphi 프로젝트 옆에 `<Project>.delphilsp.json`이 있으면 아래 규칙으로 `.omp/lsp.json`을 직접 쓴다(`RADAgent.OmpLaunch.EnsureDelphiLsp`). 손으로 만들 필요는 이 저장소를 고칠 때뿐이다.
