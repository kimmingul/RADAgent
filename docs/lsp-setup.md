...
DelphiLSP 37.0(Win64) 기준: definition과 diagnostics는 된다. references는 `-32601 Method not found`로 거절되고, hover는 `-32603 Internal server error`로 실패한다. 참조 검색은 `grep`으로 한다.
...
2. `src/RADAgent.delphilsp.json`은 IDE에서 `src\RADAgent.dproj`를 Generate LSP Config를 켠 채로 열면 생긴다.
