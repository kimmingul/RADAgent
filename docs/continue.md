...
| clangd (C++ LSP) | clangd 23.1.0으로 평가 후 연결. 64-bit IDE(Win64x): `.omp\lsp.json`, `compile_commands.json`(타깃 `x86_64-w64-windows-gnu`, `-std=c++23`, Embarcadero 매크로, `_DEBUG`) 생성, 채팅에서 omp가 lsp로 `Total` 정의(16줄)·참조, 진단의 알려진 오류 3개를 무시 대상으로 구분. 직접 LSP 시험: 오타(`Labl1`→고치기 제안), 없는 멤버(`Captin`), 인자 형식, 문법 오류를 잡고 정상 속성 사용은 오류 없음, `Caption` 정보·이동은 없음. 32-bit IDE(Win32 bcc32c): `-Xdriver`로 매크로를 읽고 `i686-pc-windows-msvc`로 같은 오류 검출(gnu 타깃은 `__int8`, `__forceinline`에서 멈춤), 채팅 lsp 정의 확인. 오류 창 없음 |
| 구버전 대비 (10.4~12) | 13.2에서만 확인: `{$LIBSUFFIX AUTO}`로 `RADAgent370.bpl`이 그대로 나옴, `scripts\build.ps1`(레지스트리로 릴리스 찾기, `-Version`), WebView2를 `RADAgent.WebView2Api` 선언으로 바꾼 뒤 64-bit·32-bit IDE 모두 채팅 왕복(WIN32-OK, VTABLE-OK), 시험 ALL PASSED. 10.4·11·12 빌드와 실행은 **미확인**: 그 IDE가 이 PC에 없음 |
| omp 명령·기능 전체 | 64-bit IDE, Smoke, 채팅 페이지를 WebView2 원격 디버깅(CDP)으로 조작. `/hotkeys` 시트, `/goal` → 터미널 전용 알림, `!echo` → 출력, 긴 턴(ping 도구) 중 Enter 끼워 넣기 + Ctrl+Enter 턴 뒤 보내기 → 답 "DONE, STEERED" 뒤 "FOLLOWED", `/copy` → 클립보드 "FOLLOWED", `/hub`, `/tree`(● 경로)·`/branch` → 고른 메시지가 입력칸, `/clear` → 이름 유지 알림, `/delete` → 세션 파일·폴더 삭제, 재시도 취소 → `abort_retry` 성공, 하위 에이전트 줄 → 대화 시트, 컨텍스트 원 → 컨텍스트 3%·5시간 2%·주간·Fable 한도와 재설정 시각, 고급 → 자동 재시도 끄기 → overlay `retry.enabled: false`, omp 재시작. 오류 창 없음 |
| 폼 편집 방식 설정 | 64-bit IDE, Smoke: 설정 → 고급 → 폼 편집 방식 "디자이너만" → 확인 → omp 재시작, `.omp\radagent-ide.json` 생김, `set_host_tools`에 `rad.form_text_edit` 없음(`rad.form_screenshot`은 있음). "자동"으로 되돌리면 파일 삭제, 도구 다시 등록. 오류 창 없음 |
| 폼 스크린샷·텍스트 일괄 수정 | omp 18.2.11이 host_tool_result의 image 부분을 모델에 넘김을 먼저 확인(빨간 사각형 PNG를 정확히 설명). 64-bit IDE, VCL Smoke: `rad.form_screenshot` → 336×159 PNG, 모델이 "Run" 버튼과 "Total=?"를 설명. `rad.form_text_edit` 한 번에 Button1 Caption='Start', Label1 `Font.Style = [fsBold]` → `.dfm` 반영, 다음 `rad.form_properties`가 Caption=Start(디자이너 다시 읽음). 없는 속성 `Foo` → "Label1 has no property Foo"로 거절, 파일 그대로. FMX(KeyboardToolbar 예제): 320×460 캡처, 모델이 스위치·버튼을 설명. 오류 창 없음 |
| clangd 설치 단추 | 64-bit IDE: 설정 → 고급 → `clangd 설치` → 확인 창 → 뒤에서 GitHub 최신 릴리스(22.1.6)를 받아 `%LOCALAPPDATA%\RADAgent\clangd\22.1.6\...\clangd.exe`, 경로 칸과 상태 줄이 채워짐, IDE는 받는 동안 멈추지 않음. 확인 → omp 다시 시작 → `.omp\lsp.json`이 그 clangd를 가리키고 채팅에서 lsp 정의(16줄). 오류 창 없음 |
| C++Builder | 64-bit IDE, `SmokeCpp.cbproj`(Win64x): 원인 확인 — C++ 디자이너의 `CreateMethod`는 이름만 등록하고 코드를 안 쓰며, IDE가 저장 때 빈 핸들러를 지워 이벤트가 끊겼다(Delphi도 빈 스텁이 저장 때 지워져 `OnClick`이 사라짐을 확인). 고친 뒤: `rad.form_apply` events로 `.h` 선언·`.cpp` 본문·`.dfm` `OnClick` 모두 생김. `rad.new_module form AboutForm` → `Unit2.cpp/.h/.dfm`(`TAboutForm`), 전에는 "Declaration of class TAboutForm is missing" 오류 창(Delphi도 같은 오류였음, 고침). 일부러 넣은 `int x = ;` → `rad.compile`이 `MainForm.cpp 26:10 expected expression`(전에는 `Build failed`만). 디버거: 중단점 → 실행 → 12줄 정지, 호출 스택 `wWinMain`, `1+2`=3, 종료. 32-bit IDE(Win32): 이벤트 핸들러 생성, `bcc32c`로 오류 줄·열. Delphi 64-bit: 이름 준 새 폼과 이벤트 스텁 확인. cquery(LLVM 5)는 VCL 헤더에서 멈춰 C++ LSP는 연결하지 않음. 오류 창 없음 |
| IDE 두 개 동시 실행 | 원인: 모든 IDE의 omp가 `%TEMP%\RADAgent\omp.stderr.log`를 stderr로 잡아 두어, 두 번째 IDE의 omp가 `프로세스 시작 실패 32`(공유 위반)로 뜨지 않았다. 고침: `Options.ProcessTempFile` — stderr 로그, `omp-host`, `project-guide`, `btw-guide`, 호환성 검사 파일을 `이름-p<IDE pid>.확장자`로 쓰고, 끝난 IDE의 파일과 예전 공용 이름은 처음 쓸 때 지운다(쓰는 중인 파일은 지워지지 않음). `rpc.log`는 공용 그대로. 64-bit·32-bit IDE를 동시에 띄워 두 채팅 모두 연결됨, 그 상태에서 `build-tests` ALL PASSED(실제 omp 시작 시험 포함). 오류 창 없음 |
| 설정 창 등 IDE 테마 연동 | 원인: 설정 창·선택/확인/입력 창·승인 창이 `IOTAIDEThemingServices.RegisterFormClass` 없이 만들어져(선택 창은 검정 고정) IDE 테마를 받지 못했다. 고침: `ChatTheme.TAgentForm`(우리 대화상자 전용 클래스)과 `ThemeForm`(클래스 등록 → `ApplyTheme`, 페이지가 가진 컨트롤까지, 회색 안내문은 muted 색, Windows 제목 줄도 어둡게). omp 호환성 검사 결과는 `MessageDlg` 대신 테마를 따르는 `ShowReport`. 32-bit IDE(어두운 테마): 설정 창 다섯 탭, 호환성 검사 창, omp 다시 시작 확인 창이 어둡고 제목 줄도 어두움. 오류 창 없음 |
| provider 로고 | node로 규칙 확인(claude→claude 회전, ollama/nemotron→nvidia, gemma/glm/kimi/qwen, openai-codex/gpt→openai 회전, openrouter/meta-llama→meta, gitlab-duo-agent→gitlab-duo 항목). 64-bit IDE: 모델 단추·검색 목록(`ollama/` → Ollama 묶음, 제작사 로고), nemotron으로 바꿔 작업 중 NVIDIA 로고 맥박, 답 위 `nemotron-3-super:cloud` 꼬리표, 설정 계정·모델 목록과 로그인 목록 로고. 32-bit IDE: 역할별 모델(입력 가능한 콤보, 목록에 로고), `/model` 창 로고, Claude 작업 중 로고 회전, 답 꼬리표. 오류 창 없음. 64-bit IDE 두 개를 동시에 띄우면 omp가 `omp.stderr.log`를 공유하지 못해 뒤 IDE의 omp가 시작하지 않는다(기존 제약) |
