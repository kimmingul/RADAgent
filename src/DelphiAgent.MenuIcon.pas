unit DelphiAgent.MenuIcon;

{ The DelphiAgent icon (resources\MenuIcon-16.png and -32.png, linked as RCDATA from
  DelphiAgentIcons.rc) in the IDE image list, for the main-menu entry. Both sizes go in so the
...
uses
  System.SysUtils, System.Classes, Winapi.Windows, Vcl.Graphics, ToolsAPI, DelphiAgent.Options;
...
begin
  ...
  try
    try
      Small := LoadPng('DELPHIAGENT_ICON16');
      Large := LoadPng('DELPHIAGENT_ICON32');
      if (Small <> nil) and (Large <> nil) and Supports(BorlandIDEServices, INTAServices280, Services) then
        GIndex := Services.AddImage('DelphiAgent.Chat', [Small, Large]);
    except
      on E: Exception do
