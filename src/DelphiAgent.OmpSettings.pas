unit DelphiAgent.OmpSettings;

{ omp settings DelphiAgent may change, scoped to one project. Values are written to
  <project>\.omp\delphiagent.yml (JSON is valid YAML) and passed to omp with --config, so the
...
uses
  System.JSON, DelphiAgent.OmpCatalog;
...
uses
  System.SysUtils, System.Classes, System.IOUtils, DelphiAgent.OmpCli;

function OverlayPath(const ProjectDir: string): string;
begin
  Result := TPath.Combine(TPath.Combine(ProjectDir, '.omp'), 'delphiagent.yml');
end;
