...
uses
  ...
  System.IOUtils, DelphiAgent.EditorContext, DelphiAgent.IdeContext, DelphiAgent.ChatPlan;
...
begin
  ...
  if Session.StartError <> '' then
    Exit('오류: ' + Session.StartError);
  if Session.Client.LinkError <> '' then
    Exit('오류: ' + Session.Client.LinkError);
  IsError := False;
...
begin
  ...
  { DelphiAgent's own; omp's RPC command list has no /btw. }
  Item := TJSONObject.Create;
  Item.AddPair('name', 'btw');
  Item.AddPair('description', '곁가지 질문: 지금 대화에 넣지 않고 따로 답을 받음 (작업 중에도 가능)');
  Item.AddPair('hint', '<질문>');
