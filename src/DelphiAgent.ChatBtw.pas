unit DelphiAgent.ChatBtw;

{ /btw side questions. Each question runs in its own omp child (DelphiAgent.BtwRunner) forked
  ...
  in the BTW notes (DelphiAgent.BtwStore), listed on the page panel. Main thread only. }
...
uses
  ...
  Winapi.Windows, Vcl.ExtCtrls, DelphiAgent.BtwStore, DelphiAgent.BtwRunner, DelphiAgent.ChatSession,
  DelphiAgent.IdeContext, DelphiAgent.Options, DelphiAgent.AgentSettings;
...
begin
  ...
  if Dir = '' then
  begin
    ChatSession.Notice('warn', '/btw는 프로젝트를 연 뒤에 쓸 수 있습니다.');
    Exit;
  end;
  if (TopicId <> '') and GActive.ContainsKey(TopicId) then
  begin
    ChatSession.Notice('warn', '이 곁가지 질문은 아직 답을 받는 중입니다.');
    Exit;
  end;
  if TopicId <> '' then
  begin
    ...
    begin
      ChatSession.Notice('warn', '곁가지 질문 메모를 찾지 못했습니다.');
      Exit;
    end;
  end
  else
