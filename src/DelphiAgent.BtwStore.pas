unit DelphiAgent.BtwStore;

{ Side questions (/btw) kept apart from the conversation, one JSON file per topic under
  %LOCALAPPDATA%\DelphiAgent\btw\<project key>\, with the topics' own omp sessions in sessions\.
...
function BtwDir(const ProjectDir: string): string;
begin
  Result := IncludeTrailingPathDelimiter(GetEnvironmentVariable('LOCALAPPDATA')) + 'DelphiAgent\btw\' +
