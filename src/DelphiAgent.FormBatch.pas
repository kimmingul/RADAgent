unit DelphiAgent.FormBatch;

{ rad.form_apply: many designer changes (delete, create, properties, events) shown as one list
  and approved once, then applied in order through DelphiAgent.FormEdits. Never saves.
  Main thread only. }
...
uses
  System.SysUtils, System.Classes, System.JSON, ToolsAPI, DelphiAgent.FormDesigner,
  DelphiAgent.FormEdits;
      ...
      for Value in Components do
        ...
        begin
          ...
          if FindNative(Editor, Item.GetValue<string>('name', '')) <> nil then
            Head := '* 수정 ' + Item.GetValue<string>('name', '')
          else
            Head := Format('+ 추가 %s: %s  부모=%s', [Item.GetValue<string>('name', ''),
              Item.GetValue<string>('class', ''), Item.GetValue<string>('parent', '(폼)')]);
          Lines.Add(Head);
