...
- LSP (clangd): RAD Studio에는 omp가 쓸 C++ LSP가 없다(설치본 `cquery.exe`는 LLVM 5 기반이라 VCL 헤더에서 멈춘다). [clangd](https://github.com/clangd/clangd/releases)(LLVM)를 설치하고 설정 → 고급 → clangd에 경로를 넣거나 PATH에 두면, omp를 시작할 때 `.omp\clangd\compile_commands.json`과 `.omp\lsp.json`을 만든다. 컴파일 옵션은 활성 플랫폼 컴파일러가 실제로 쓰는 헤더·타깃·표준·매크로(`-###`, `-dM -E`)와 프로젝트 IncludePath·Defines에 `-D__published=public`을 더한 것이다. Win32(bcc32c)는 `i686-pc-windows-msvc`, Win64x(bcc64x)는 `x86_64-w64-windows-gnu`로 읽는다. clangd가 없으면 한 번 알리고 grep/read로 찾는다.
