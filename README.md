...
- 입력 상자: Enter 보내기, Shift+Enter 줄바꿈, ↑ 이전 문장, `/` 명령 목록. 작업 중에는 보내기 단추가 중지(■)로 바뀐다. 중지는 omp에 abort를 보내고, 5초 안에 턴이 끝나지 않거나 한 번 더 누르면 omp를 강제로 끝내고 같은 대화로 다시 시작한다(응답하지 않는 omp 대비). 위에 활성 파일, `+ 선택 N–M줄`(누르면 선택 영역을 붙임), 저장 안 한 파일 수.
